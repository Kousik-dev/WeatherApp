...
function Landing() {
  ...
  async function handleCity(e: FormEvent) {
    ...
    await api.get(`?q=${city}&appid=e81965343df414cbfb25d98c8741fe2a&units=metric`).then(response => {
  ...
  async function handleCityByCordinates (lat: number, lon: number) {
    await api.get(`?lat=${lat}&lon=${lon}&appid=e81965343df414cbfb25d98c8741fe2a&units=metric`).then(response => {
      setValid(true)
      setData(response.data)
    })
  }
            ...
            <div className="header">
              <form onSubmit={handleCity}>
                <div className="extras">
                  ...
                  {!valid && (
                    <span className="snackbar">Invalid Option</span>
                  )}
                </div>
